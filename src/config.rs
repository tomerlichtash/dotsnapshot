...
impl Config {
    ...
    fn get_config_paths() -> Vec<PathBuf> {
        ...
        if let Some(home_dir) = dirs::home_dir() {
            paths.push(home_dir.join(".config").join("dotsnapshot").join("config.toml"));
    ...
    pub fn get_output_dir(&self) -> PathBuf {
        let path = self.output_dir.clone().unwrap_or_else(|| PathBuf::from("./snapshots"));
...
mod tests {
    ...
    async fn test_config_load_and_save() -> Result<()> {
        ...
        assert_eq!(loaded_config.get_output_dir(), PathBuf::from("/tmp/snapshots"));
        assert_eq!(loaded_config.get_include_plugins(), Some(vec!["homebrew".to_string(), "vscode".to_string()]));
    ...
    async fn test_config_paths() {
        ...
        assert!(paths.iter().any(|p| p.file_name().unwrap() == "dotsnapshot.toml"));
    }
}