...
pub(crate) async fn handle_add_hook(
    target: HookTarget,
...
pub(crate) async fn handle_remove_hook(
    target: HookTarget,
    index: Option<usize>,
    all: bool,
...
pub(crate) async fn handle_list_hooks(
...
pub(crate) async fn handle_validate_hooks(
    plugin: Option<String>,
    pre_plugin: bool,
    post_plugin: bool,
    pre_snapshot: bool,
...
pub(crate) async fn handle_scripts_dir(
    set: Option<PathBuf>,
    create: bool,
    config_path: Option<PathBuf>,
) -> Result<()> {
...
pub(crate) fn convert_action_args_to_hook_action(args: HookActionArgs) -> Result<HookAction> {
...
pub(crate) fn determine_hook_target(target: &HookTarget) -> Result<(String, Option<String>)> {
...
pub(crate) async fn load_or_create_config(config_path: Option<PathBuf>) -> Result<Config> {
...
pub(crate) fn get_config_file_path(config_path: Option<PathBuf>) -> PathBuf {
...
pub(crate) fn ensure_global_config(config: &mut Config) {
...
pub(crate) fn ensure_plugin_config(config: &mut Config, plugin_name: &str) {
...
pub(crate) fn modify_plugin_config<F, R>(
    config: &mut Config,
    plugin_name: &str,
    modifier: F,
) -> Option<R>
...
pub(crate) async fn handle_plugin_hook_removal(
    config: &mut Config,
    plugin_name: &str,
    hook_type: &str,
    index: Option<usize>,
...
pub(crate) fn show_global_hooks(
    config: &Config,
    pre_snapshot: bool,
    post_snapshot: bool,
    verbose: bool,
...
pub(crate) fn show_plugin_hooks(
    config: &Config,
    plugin_name: &str,
    pre_plugin: bool,
    post_plugin: bool,
...
pub(crate) fn show_all_plugin_hooks(
    config: &Config,
    pre_plugin: bool,
    post_plugin: bool,
    verbose: bool,
...
pub(crate) fn validate_hook_list(
...
pub(crate) fn count_total_hooks(config: &Config) -> usize {
    let mut total = 0;
...
pub(crate) fn get_all_plugin_names(config: &Config) -> Vec<String> {
...
pub(crate) async fn count_scripts_in_directory(dir: &PathBuf) -> Result<usize> {
    let mut count = 0;
